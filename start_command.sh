...
export BROWSER="${BROWSER:-/usr/local/bin/openclaw-browser}"
...
text/html=openclaw-browser.desktop
x-scheme-handler/http=openclaw-browser.desktop
x-scheme-handler/https=openclaw-browser.desktop
application/xhtml+xml=openclaw-browser.desktop
EOF
cat >/root/.config/xfce4/helpers.rc <<'EOF'
WebBrowser=custom-WebBrowser
WebBrowserCustom=/usr/local/bin/openclaw-browser
EOF
...
if command -v xdg-settings >/dev/null 2>&1; then
    xdg-settings set default-web-browser openclaw-browser.desktop >/tmp/xdg-settings.log 2>&1 || true
fi
